...
const recursionPattern = (int1, int2) => {
    let arr = [];
    (function lmao(int1, int2) {
      if (int1 <= 0) int2 = -int2;
      arr.push(int1);
      if (int1 == arr[0] && int2 < 0) return 0;
  
      return lmao(int1 - int2, int2);
    })(int1, int2);
    return arr;
  };
// -------------------------------------------------------------------------------------------------------
...
const isPalindrome = (str) => {
    if(str.length>2){
        let x=(str.match(/\w/ig))
        let start=x.slice(0,(Math.floor(x.length/2))).join("").toLowerCase()
        let end=x.slice((Math.ceil(x.length/2)),).reverse((a,b)=>(a-b)).join("").toLowerCase()
        if(start===end){
            return true
        }
        else {
